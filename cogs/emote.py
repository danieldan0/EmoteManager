...
class Emotes(commands.Cog):
	...
	@commands.bot_has_permissions(attach_files=True)
	async def archive(self, context):
		"""Outputs an HTML file containing all of the server's emotes."""

		elements = []
		for emoji in context.guild.emojis:
			elements.append(f'\t<img title=":{emoji.name}:" alt=":{emoji.name}:" src="{emoji.url}">')
